    ...
    public static void downloadGraph(Context context, int city, OnRefreshTaskFinish listener){
        ...
        switch (city){
            ...
            case Constants.WARSAW:
                url = "http://www.meteo.pl/um/metco/mgram_pict.php?ntype=0u&fdate=" + date + "00&row=406&col=250&lang=pl";
                break;
            case Constants.LODZ:
                url = "http://www.meteo.pl/um/metco/mgram_pict.php?ntype=0u&fdate=" + date + "00&row=418&col=223&lang=pl";
                break;
        }
        new ImageDownloaderTask(context, city, listener).execute(url);
    }
    ...
    private static class ImageDownloaderTask extends AsyncTask<String, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(String... params) {
            ...
            if (isCancelled() || bitmap == null) {
                ...
                saveBitmapToFile(bitmap, String.valueOf(city)); //save in background
        ...
        @Override
        protected void onPostExecute(Bitmap bitmap) { //TODO scale image to fit widget and release memory
            showToast(context, bitmap != null? "Graph refreshed" : "Refresh failed");

            if(listener != null)
                listener.onRefreshTaskFinished(bitmap); //TODO refresh widget if null
        }
